...
def get_candle_state_emoji(candles_df: pd.DataFrame) -> str:
    ...
    if candle_range > 0 and body_size / candle_range < 0.15: # Узкое тело
        # Здесь в идеале нужно проверять объем. Без объема просто узкое тело.
        # Для сквота (Squat) нужен объем. Если нет объема, пока не детектируем.
        # Заглушка, если нет данных по объему:
        # if current_candle.get('volume', 0) > candles_df['volume'].rolling(window=20).mean().iloc[-1]:
        #     return '🔷' # Синий ромб

        pass # Пока не можем точно определить Squat без объема
